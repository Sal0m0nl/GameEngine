...
ResourceManager::ShaderProgramsMap ResourceManager::m_shaderPrograms_map;
...
std::shared_ptr<Render::Texture2D> ResourceManager::loadTexture(const std::string &name,
    ...
    std::shared_ptr<Render::Texture2D> newTexture = m_textures.emplace(name, std::make_shared<Render::Texture2D>(width, height, pixels, channels, GL_LINEAR, GL_CLAMP_TO_EDGE)).first->second;
...
std::shared_ptr<Render::Sprite> ResourceManager::loadSprite(const std::string &spriteName,
    ...
    if (!p_ShaderProgram) {
        std::cerr << "Error while loading sprite's shader with name: " << shaderName << std::endl;
        return nullptr;
    }
...
std::shared_ptr<Render::AnimatedSprite> ResourceManager::loadAnimatedSprite(const std::string &spriteName,
    ...
    std::shared_ptr<Render::Texture2D> p_Texture = getTexture(textureName);
    if (!p_Texture) {
        std::cerr << "Error while loading animated sprite with name: " << spriteName << std::endl;
        return nullptr;
    }
    std::shared_ptr<Render::ShaderProgram> p_ShaderProgram = getShaderProgram(shaderName);
    if (!p_ShaderProgram) {
        std::cerr << "Error while loading animated sprite's shader: " << shaderName << std::endl;
        return nullptr;
    }

    std::shared_ptr<Render::AnimatedSprite> newAnimatedSprite = m_AnimatedSpritesMap.emplace(
                                                                spriteName,
                                                                std::make_shared<Render::AnimatedSprite>(p_Texture,
                                                                                                           subTextureName,
                                                                                                           p_ShaderProgram,
                                                                                                            glm::vec2(0.0f, 0.0f),
...
std::shared_ptr<Render::ShaderProgram> ResourceManager::getShaderProgram(const std::string &shader_name) {
...
std::shared_ptr<Render::Texture2D> ResourceManager::getTexture(const std::string &texture_name) {
...
std::shared_ptr<Render::Sprite> ResourceManager::getSprite(const std::string &sprite_name) {
...
std::shared_ptr<Render::AnimatedSprite> ResourceManager::getAnimatedSprite(const std::string &sprite_name) {
    AnimatedSpritesMap::const_iterator it = m_AnimatedSpritesMap.find(sprite_name);
    if (it != m_AnimatedSpritesMap.end()) {
        return it->second;
    }
    std::cerr << "Can't get animated sprite with name " << sprite_name << std::endl;
    return nullptr;
}

std::string ResourceManager::getStringFromFile(const std::string& relative_path_to_file) {
